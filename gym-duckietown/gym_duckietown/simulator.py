...
class Simulator(gym.Env):
    ...
    def reset(self):
        ...
        # self.cur_pos = np.ndarray([3,0,3])
        # self.cur_pos[0] = 2
        # self.cur_pos[2] = 7
        print(f"start: {self.cur_pos[0]}")
    ...
    def get_occupancy_grid(self, map_data: dict):
        ...
        for j, row in enumerate(tiles):
            ...
            for i, tile in enumerate(row):
                ...
                if drivable:
                    img[j * no_pixels_grid_side:(j + 1) * no_pixels_grid_side,
    ...
    def _interpret_map(self, map_data: dict):
        #TODO: fix Hard code for start pos
        map_data['start_tile'] = (5,3)
        if not 'tile_size' in map_data:
